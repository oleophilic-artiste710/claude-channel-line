...
Claude 可以用以下方式回覆 LINE 使用者：

| 工具 | 類型 | 說明 |
|------|------|------|
| `reply` | 文字 | 純文字訊息（超過 5,000 字自動分割，最多 5 則） |
| `reply_image` | 圖片 | 透過 HTTPS URL 傳送圖片 |
| `reply_flex` | Flex Message | 豐富排版卡片，支援按鈕、連結、圖文混排、輪播（bubble / carousel） |
| `reply_mixed` | 混合 | 一次傳送最多 5 則不同類型的訊息 |

> **提示**：`reply_token` 在收到訊息後 30 秒內有效，逾時後 Claude 會自動改用 `user_id` 進行 push。
